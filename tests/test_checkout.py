...
def test_shipping_get(client, auth, app):
...
def test_shipping_post(client, auth, app):
    with app.app_context():
        auth.register()
        auth.login()
...
def test_copy_to_orders(client, app, search, cart, auth):
    with app.app_context():
        db = get_db()
        auth.register()
        auth.login()
...
def test_delete_old_items(client, app, search, cart, auth):
    with app.app_context():
        db = get_db()
        auth.register()
        auth.login()
...
def test_checkout_post_logged_in(client, app, search, cart, auth):
    with app.app_context():
        auth.register()
        auth.login()
        session_id = "testtesttesttesttesttesttesttest"
...
def test_checkout_post_not_logged_in(client, app, search, cart):
    with app.app_context():
        session_id = "testtesttesttesttesttesttesttest"
        supplier_id = search.insert_supplier()
        category_id = search.insert_category()
...
def test_checkout_get_logged_in(client, app, search, cart, auth):
    with app.app_context():
        auth.register()
        auth.login()
        session_id = "testtesttesttesttesttesttesttest"
...
def test_checkout_get_not_logged_in(client, app, search, cart):
    with app.app_context():
        session_id = "testtesttesttesttesttesttesttest"
        supplier_id = search.insert_supplier()
        category_id = search.insert_category()
